...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 403 when there have no room capacity available", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const hotel = await createHotel();
...
describe("PUT /booking/:bookingId", () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 403 when there have no room capacity available", async () => {
      ...
      await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const room = await createRoomWithHotelId(hotel.id);
      const finalRoom = await createRoomWithHotelId(hotel.id);
      const booking = await createBooking(user.id, room.id);
      
      await createTwoBookings(finalRoom.id);

      const response = await server.put(`/booking/${booking.id}`).set("Authorization", `Bearer ${token}`).send({ roomId: finalRoom.id });
    ...
    it("should respond with status 404 when room doesn't exist", async () => {
      ...
      const response = await server.put("/booking/1").set("Authorization", `Bearer ${token}`).send({ roomId: 2139 });
    ...
    it("should respond with status 403 when roomId is invalid", async () => {
      ...
      await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const room = await createRoomWithHotelId(hotel.id);
      const bookingIds = await createTwoBookings(room.id);

      const response = await server.put(`/booking/${bookingIds[0]}`).set("Authorization", `Bearer ${token}`).send({ roomId: 0 });
    ...
    it("should respond with status 403 when the user doesn't have a booking", async () => {
      ...
      const response = await server.put("/booking/1").set("Authorization", `Bearer ${token}`).send({ roomId: finalRoom.id });
      
      expect(response.status).toBe(httpStatus.FORBIDDEN);
    });

    it("should respond with status 403 when the user is not the owner of the booking", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const hotel = await createHotel();
      ...
      const response = await server.put("/booking/255").set("Authorization", `Bearer ${token}`).send({ roomId: finalRoom.id });
    ...
    it("should respond with status 200 and booking id", async () => {
      ...
      expect(user.id).toBe(updatedBooking.userId);
    });
  });
});
